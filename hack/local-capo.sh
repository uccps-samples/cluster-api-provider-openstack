...
./hack/cvo-unmanage.py uccp-machine-api machine-api-operator

# Scale the operator down to zero so it doesn't 'fix' machine-api-controllers
oc -n uccp-machine-api scale deploy/machine-api-operator --replicas 0

# Ensure machine-api-controllers deployment is annotated with
# last-applied-configuration
oc -n uccp-machine-api get deploy/machine-api-controllers -o json | \
    oc apply -f -
...
# Run MAPO locally
./machine-controller --logtostderr=true --v=3 --namespace=uccp-machine-api
